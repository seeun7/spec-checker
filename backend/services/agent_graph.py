...
async def node_analyze_vision(state: AgentState) -> dict: # 이미지가 소프트웨어인지 파악하고 이름을 추출
    ...
    user_msg = HumanMessage(content=[
        {"type": "text", "text": "이 이미지에 나타난 소프트웨어의 이름은 무엇인가요?"},
...
async def node_generate_comparison(state: AgentState) -> dict: # 사용자의 PC 사양과 검색된 권장 사양을 비교 분석하여 결과를 생성
    ...
    sys_msg = SystemMessage(content=(
        ...
        "4. 검색 결과에 하드웨어 정보가 누락되어 있다면 방대한 IT 지식을 이용해 해당 소프트웨어의 보편적인 권장 사양을 적어 넣으세요."
    ))
    # 사용자 프롬프트
    prompt = f"""
    ...
    1. 사양 비교표 - 운영체제, CPU, RAM, GPU 항목을 기준 행으로 삼아 사용자 사양과 권장 사양을 비교하는 표를 그리세요. 반드시 아래의 마크다운 표 양식을 그대로 복사해서 빈칸을 채워 출력하세요. (가운데 |---|---|---| 구분선을 절대 빼먹지 마세요.)
    ...
    | RAM |  |  |
    | GPU |  |  |
    2. 최종 분석 결과 - 표 아래에 사용자의 PC 환경에서 해당 소프트웨어를 원활하게 실행할 수 있는지, 버벅거리는지, 너무 무거운지 등을 간단명료하게 작성하세요. 
    """
